...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, ChangePasswordModalComponent, SyliSpinnerComponent],
  template: `
      ...
      @if (!loading() && !editMode() && profile()) {
        <div class="card space-y-6">
          <div class="flex items-center gap-4">
            <div class="w-16 h-16 rounded-full bg-violet-100 flex items-center justify-center text-violet-700 font-bold text-2xl">
              {{ initials() }}
            </div>
            <div>
              ...
              <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-violet-100 text-violet-800 mt-1">
                Secrétaire
              </span>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            ...
            <div class="sm:col-span-2">
              <p class="text-gray-400 font-medium uppercase tracking-wide text-xs mb-1">Notes</p>
  ...
  save(): void {
    ...
    this.profileService.updateMyProfile({
      ...
      next: (updated) => {
        ...
        this.authService.updateProfileNames(updated.firstName, updated.lastName);
